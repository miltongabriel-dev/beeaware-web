-- Re-verify after deploying the ingest-security-sources function (the
-- first verification in 20260907160000 ran BEFORE the function was
-- actually deployed via `supabase functions deploy` — db push only
-- pushes SQL migrations, not edge function code, so FrCrimeAdapter/
-- FrNewsAdapter weren't even registered yet when that one ran).
do $$
declare
  r record;
  crime_events int;
